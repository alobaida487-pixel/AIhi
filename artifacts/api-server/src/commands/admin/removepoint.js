...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        if (type === 'xp') {
            ...
            db.set(`level_${user.id}`, { xp: Math.max(0, levelll.xp - amount), nid: user.id });
        } else if (type === 'poi') {
            let levelll = db.get(`support_${user.id}`) || { poi: 0, id: user.id };
            db.set(`support_${user.id}`, { poi: Math.max(0, levelll.poi - amount), id: user.id });
        }

        message.reply(`**Done Removed __${amount}__ ${type.toUpperCase()} From ${user}**`);
    }
};
